...
class MetsHandler:
    ...
    def build_contributors(self, author_entries, author_table):
        ...
        for contrib in contrib_mets:
            check_name = contrib[
                'person_name sequence="additional" contributor_role="author"'
            ]["surname"].lower().strip().replace(" ", "").replace(".", "") + contrib[
                'person_name sequence="additional" contributor_role="author"'
            ][
                "given_name"
            ].lower().strip().replace(
            ...
            if check_name in author_info:
                ...
                if author_info[check_name]["orcid"] != "":
                    contrib[
                        'person_name sequence="additional" contributor_role="author"'
                    ].update({"ORCID": author_info[check_name]["orcid"]})
            ...
            modified_citations_list.append(
                {
                    'citation key="ref='
                    + str(refcount)
                    + '"': {"unstructured_citation": self.entry_normalizer(citation)}
                }
            )
            refcount += 1
    ...
    def build_serials_xml(self, custom_filename):
        ...
        if self.mets_type == "serials":
            ...
            with open("xml_templates/" + self.mets_type + "_template.xml") as f:
                ...
                xml = xml.replace("{{ doi_batch_id }}", batch_uuid).replace(
                    "{{ timestamp }}", timestamp
                )
                f.close()
